...
func (ms mapSeq) complementSimbol(with byte) {
	ms[with]--
}
...
func main() {
	...
	strMap := strToMap(str)
	diffSef := getBaseTestSeq(strMap, numSimbols)
	minLen := int(^uint(0) >> 1)
	// fmt.Println(diffSef, diffSef.len())
	for i := 0; i < len(str)-diffSef.len(); i++ {
		buffSef := strToMap(str[i : i+diffSef.len()])
		buffSef.complement(diffSef)
		if checkSeq(buffSef) {
			curLen := diffSef.len()
			if curLen < minLen {
				minLen = curLen
			...
			for j := i + diffSef.len(); j < len(str) && j-i+1 < minLen; j++ {
				fmt.Println(j - i + 1)

				buffSef.complementSimbol(str[j])
				if checkSeq(buffSef) {
					curLen := j - i + 1
					if curLen < minLen {
						minLen = curLen
...
func getBaseTestSeq(strMap mapSeq, length int) mapSeq {
	...
	for i := 0; i < len(genomeCodes); i++ {
		value, exists := strMap[genomeCodes[i]]
